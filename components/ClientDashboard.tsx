...
export const ClientDashboard = ({ client, onLogout, allServices, products, onRequestService, onCancelService, onSendMessage, onRateService }: { client: Client, onLogout: () => void, allServices: Service[], products: Product[], onRequestService: (service: Omit<Service, 'id' | 'value'>) => void, onCancelService: (serviceId: string) => void, onSendMessage: (serviceId: string, sender: 'client' | 'electrician', message: string) => void, onRateService: (serviceId: string, rating: number, ratedBy: 'client' | 'electrician') => void }) => {
    
    const [clientView, setClientView] = useState<'service' | 'store'>('service');
